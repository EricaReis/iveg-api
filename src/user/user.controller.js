...
const userController = {
  ...
  editUser(req) {
    return new Promise(async (resolve, reject) => {
      ...
      userActions.editUser(req).then(user => {
        if (user) {
          resolve(
            createResponse(200, `Usuário ${user.name} alterado com sucesso`)
          );
        } else {
  ...
  deleteUser(id) {
    return new Promise(async (resolve, reject) => {
      ...
      userActions.deleteUser(id).then(user => {
        if (user) {
          resolve(
            createResponse(200, `Usuário ${user.name} deletado com sucesso`)
          );
        } else {
          resolve(createResponse(404, 'usuário não encontrado'));
        }
